...
@Tag("unit")
@DisplayName("Board initialization & rendering")
public class BoardInitTest {

    @Test
    @DisplayName("Start position: 12 white & 12 black on correct rows")
    void starts_with_12_white_and_12_black_on_correct_rows() {
        // Given a fresh board
        Board b = new Board();

        // When we inspect pieces
        ...
        // Then the counts and placements match the rules
    ...
    @Test
    @DisplayName("Render shows grid and file letters")
    void board_render_has_axes_and_grid() {
        // Given a fresh board
        Board b = new Board();

        // When we render without color
        String out = b.render(false);

        // Then grid and axes are visible (smoke checks)
